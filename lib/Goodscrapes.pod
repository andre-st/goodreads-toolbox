...
=head2 C<void> gseterr(I<{ ... }>)

=over

=item * change one or multiple error handling parameters

=item * C<retry_on_error =E<gt> bool>

=item * C<retry_on_crit =E<gt> bool>

=item * C<maxretries =E<gt> int> number of retries if ther was an error, 
        critical issues are retried indefinitely (if retry_on_crit is true)
